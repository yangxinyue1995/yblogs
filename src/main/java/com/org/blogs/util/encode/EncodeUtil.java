...
public class EncodeUtil {
	...
	private static String BestrowScreen = "*";
	...
	public static String encodeAll(String input) {
		...
		try {
			input = encodeHtmlTag(input);
			input = encodeSql(input);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return input;
	...
	public static String encodeSql(String input) {
		if (StringUtils.isEmpty(input)) {
			return input;
		}
	...
	public static boolean isContentKeyWords(String input) {
		if (StringUtils.isEmpty(input)) {
			return false;
		}
		MutiPatternParser par = MutiPatternParser.getInstance();
		return par.isContentKeyWords(input);
	}

	public static String replaceKeyWords(String input) {
		if (StringUtils.isEmpty(input)) {
			return "";
		}
		MutiPatternParser par = MutiPatternParser.getInstance();
		List<String> key = par.getTxtKeyWords(input);
		if (ArrayUtils.isNotEmpty(key.toArray())) {
			for (String string : key) {
				input = input.replaceAll(string, BestrowScreen);
			}
		}
